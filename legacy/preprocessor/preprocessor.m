...
function [coord,centelem,elem,esurn1,esurn2,nsurn1,nsurn2,bedge,inedge,...
    ...
    kmap,wells,klb,limiterflag,rowposit,nltol,maxiter,acel,midflowcompared] = preprocessor 

%--------------------------------------------------------------------------
%It reads the data file (from "Start.dat")

[head,benchkey,pathstore,numcase,complemfile,phasekey,xyrz,r0,symaxe,...
    keymsfv,coarseratio,pmethod,auxcvfactor,interptype,smethod,...
    ...
    bcflag,numbcnode,bcnodepointer,courant,totaltime,numwell,well, nltol, maxiter,acel,midflowcompared] = ...
    getdatafile;
...
elseif strcmp(keyfile,'n')  
    ...
    if exist(verifygeofile,'file') == 0
        ...
        disp('---------------------------------------------------------------');
        disp('---------------------------------------------------------------');
        disp('The *.geo file was not found!!!');
        disp('Verify either the path or the name attributed to file.');
        disp('---------------------------------------------------------------');
    end  %End of internal IF
...
[elem,nbe,nelem,nodelim,intnode,flaglim,inboundedge] = ...
...
[bedge,inedge,klb] = getinfoedge(coord,elem,nnode,nbe,nelem,nodelim,...
    intnode,flaglim,verifymshfile,bcflag);

%Gives the information of "bedge" and "inedge" generated
disp('"bedge" was generated!');
disp('"inedge" was generated!');

% utilize Tipo1Malha2.msh, Tipo1Malha3.msh
%     x=bedge(:,1);
%     y=bedge(:,2);
%     bedge(:,1)=y;
%     bedge(:,2)=x;
...
function [head,benchkey,pathstore,numcase,complemfile,phasekey,xyrz,r0,...
    symaxe,keymsfv,coarseratio,pmethod,auxcvfactor,interptype,...
    ...
    well,nltol,maxiter,acel,midflowcompared] = getdatafile
...
%8. Type of Solver for the Eliptic Equation (Pressure Equation)
getdata = textscan(readfile,'%s',1,'CommentStyle',{'//'});
...
%25.1. MUSCL 
getdata = textscan(readfile,'%c',1,'CommentStyle',{'//'});
%Attribute the data to "musclkey"
MUSCL = getdata{1};
%Is MUSCL turned on?
if strcmp(MUSCL,'y') && order > 1 && strcmp(recovtype,'ggg') == 0
    %Define if MUSCL is turned on:
    MUSCL = 'on';
...
% Foi modificado
%==========================================================================
%28. Type of the PL_MUSCL strategy
getdata = textscan(readfile,'%u',1,'CommentStyle',{'//'});
%Attribute the data to "plmuscltype"
moodtype = getdata{1};
%29. Posteriori Limitir 
getdata = textscan(readfile,'%c',1,'CommentStyle',{'//'});
%Attribute the data to "plMUSCLkey"
PLkey = getdata{1};
%Is PL_MUSCL turned on?
if strcmp(PLkey,'y') && order > 1 && strcmp(recovtype,'ggg') == 0
    %Define if PL_MUSCL is turned on:
    PL = 'on';
%There is NO Greek Correction
else
    PL = 'off';
...
%30. MOODPL Posteriori Limitir 
getdata = textscan(readfile,'%c',1,'CommentStyle',{'//'});
%Attribute the data to "moodplkey"
MOODPLkey = getdata{1};
%Is MOOD turned on?
if strcmp(MOODPLkey,'y') && order > 1 && strcmp(recovtype,'ggg') == 0
    %Define if MOODPL is turned on:
    MOODPL = 'on';
%There is NO Greek Correction
else
    MOODPL = 'off';
...
%31. U-MUSCL 
getdata = textscan(readfile,'%c',1,'CommentStyle',{'//'});
%Attribute the data to "Umusclkey"
UMUSCL = getdata{1};
%Is UMUSCL turned on?
if strcmp(UMUSCL,'y') && order > 1 && strcmp(recovtype,'ggg') == 0
    %Define if UMUSCL is turned on:
    UMUSCL = 'on';
%There is NO Greek Correction
else
    UMUSCL = 'off';
...
% a alocação do vetor serve para identificar todos os nos
% do contorno e damos flag os demais que estão no interior é zero
