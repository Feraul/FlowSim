...
function [bcattrib] = PLUG_bcfunction(vertices,flagptr,time,env,parms)

% ── Prepara coordenadas (logica comum — fica aqui) ────────────
    ...
    if isfield(env, 'benchmark') && ~isempty(env.benchmark)
        bcattrib = env.benchmark.configurarContorno(...
            coordmid, vertices, flagptr, time, env, parms);
        return
    end

    error('PLUG_bcfunction: env.benchmark nao definido.');
...
%                     vert=vertices(i,1);
%                     bcattrib(vert,1)=bcflag(1,2);
%                 elseif flagptr(i,1)==2
%                     vert=vertices(i,1);
% 
%                     bcattrib(vert,1)=bcflag(2,2) ;
%                 elseif flagptr(i,1)==3
%                     vert=vertices(i,1);
% 
%                     bcattrib(vert,1)=bcflag(3,2) ;
%                 end
%             end
%         case 439
%             for i=1:size(flagptr,1)
...
%                 elseif flagptr(i,1)==2
%                     vert=vertices(i,1);
%                     bcattrib(vert,1)=65-coordmid(vert,2) ;
%                     %bcattrib(vert,1)=bcflag(2,2);
%                 elseif flagptr(i,1)==3
%                     vert=vertices(i,1);
% 
...
%             bcattrib = ...
%                 calcCase4(coordmid,env.config.numcase,alfa,a1,a2,a3,a4,b1,b2,b3,b4);
% 
%         %------------------------------------------------------------------
...
%                 x= coordmid(1);
%                 y=coordmid(2);
%                 bcattrib = (0.8/pi)*cos(0.25*pi*(x+y+2*time))+0.5*(x+y);
%             else
%                 x=coord(vertices,1);
%                 y=coord(vertices,2);
%                 bcattrib = (0.8/pi)*cos(0.25*pi*(x+y+2*time))+0.5*(x+y);
...
%             y=coordmid(2);
% 
%             if (x==0 || x==20 )&& (y==0 || y>0)% dirichlet
%                 bcattrib =1+sin(2*x+y);
