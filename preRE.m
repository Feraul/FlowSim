...
switch env.config.numcase
    ...
    case 437
        ...
        dt=(1/8)^2; %colocar para refinamento o dt
    case 438
        % Artigo: An adaptive solution strategy for Richards’ equation
        alpha=3;
        centelem = env.geometry.centelem;
        mask = centelem(:,2) > 0.5;          % condição lógica para cada linha
  
        % Pré-alocação (opcional, mas boa prática)
        h_init = zeros(size(centelem,1), 1);
        h_old  = zeros(size(centelem,1), 1);

        % Atribuição direta com máscara lógica
        h_init(mask)  = 0.9;
        h_init(~mask) = -3;
        h_old(mask)   = 0.1;
        h_old(~mask)  = -10;
        dt=(0.1)^1; %colocar para refinamento o dt
    case 439
        theta_s=0.3;
        theta_r=0.0;
        
        centelem = env.geometry.centelem;
        mask = centelem(:,2) > 65;          % condição lógica para cada linha
        m=65-centelem(:,2);
        % Pré-alocação (opcional, mas boa prática)
        h_init = m.*ones(size(centelem,1), 1);
        h_old  = zeros(size(centelem,1), 1);
