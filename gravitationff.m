function [g]=gravitationff(kmap,gravelem)
global inedge bedge elem centelem coord dens visc
Klef=zeros(2,2);
Krel=zeros(2,2);
...
for ifacont=1:size(bedge,1)
    ...
    Keq=Klef;
    g(ifacont,1)=dens(1,1)*dot((R*ve1')'*Keq,(gravelem(lef,1:2)));
...
for iface=1:size(inedge,1)
        ...
        % vd1aux=coord(inedge(iface,2),:)-coord(inedge(iface,1),:);
         ...
         % calcular mobilidade da agua e o oleo na interface
         
         Swnleft = ((Sw(left) - satlimit(1))./(1 - satlimit(1) - satlimit(2))); 
        ...
        lambdawIJ = krw/visc(1);
        %Attribute OIL mobility
        lambdaoIJ  = kro/visc(2);
        %-------------------------------------------------------------------
        
         Keq=inv((dj1*inv(Klef)+dj2*inv(Krel))); % equation 21
         graveq=((dj1*gravelem(lef,:)+dj2*gravelem(rel,:))'); % equation 22
         g(iface+size(bedge,1),1)=(lambdawIJ*dens(1)+lambdaoIJ*dens(2))*dot(((R*vd1')')*Keq, graveq);% equation 20
