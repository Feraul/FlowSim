function [flowrate,flowresult]=ferncodes_flowrateDMP(p, pinterp, parameter,nflag,gamma,weightDMP,mobility)
global inedge coord bedge bcflag centelem phasekey 
valuemin=1e-16;
...
for ifacont=1:size(bedge,1)
    ...
    velocity(ifacont,1)=flowrate(ifacont,1)/normcont;
...
for iface=1:size(inedge,1)
    ...
    velocity(iface+ size(bedge,1),1)=flowrate(iface+ size(bedge,1),1)/norma;
    %Attribute the flow rate to "flowresult"
    %On the left:
