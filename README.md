...
│       └── tpfa.m
│
├── ★  RUNTIME CORE (still at root — active code paths)  ★
├── preprocessormod.m            mesh loader + connectivity (called by main)
├── preprocessmethod.m           per-method preprocessing wrapper
├── hydraulic.m / hydraulic_RE.m time-loop drivers (steady / Richards)
├── setmethod.m                  dispatch to appropriate time driver
├── IMPES.m / IMPEC.m / IMHEC.m  saturation / concentration coupling
├── PLUG_bcfunction.m            boundary-condition evaluator
├── PLUG_kfunction.m             permeability evaluator (per-timestep)
├── PLUG_sourcefunction.m        source term
├── PLUG_dfunction.m             dispersion / diffusion
├── addsource.m / soil_properties.m / postprocessor.m
├── solver.m                     linear system solve wrapper
├── (…and ~150 more supporting files)
│
├── ★  MESH FIXTURES  ★
├── *.msh                        16 gmsh mesh files (M8*, Hermeline*, ...)
├── Perm_Var*.mat                permeability datasets
│
├── ★  TESTS  ★
├── tests/
│   ├── README.md
...
Root `.m` file count: **~170** (was 285 before campaign).  
`.m` files under `legacy/`: **~110** (all reachable from runtime via path precedence).
